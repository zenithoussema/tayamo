...
export function ContactForm({ dict,   // eslint-disable-next-line @typescript-eslint/no-unused-vars
  ...
  if (status === "success") {
    return (
      <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-10 text-center">
        ...
        <h2 className="mb-3 text-2xl font-bold text-text">{dict.success}</h2>
        <a
          href={whatsappUrl}
          target="_blank"
          rel="noopener noreferrer"
          className="mt-6 inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-6 py-3 font-bold text-white transition-colors hover:bg-emerald-500"
        >
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-5 rounded-2xl border border-border bg-surface p-7 lg:p-8">
      ...
      <div className="flex flex-col gap-2">
        <label htmlFor="activityInterest" className="text-sm font-medium text-text-secondary">
